...
class SkipMissingDataloaderPlanner(DefaultLoadPlanner):
    def create_local_plan(self):
        try:
            return super().create_local_plan()
        except RuntimeError as e:
            # ignore dataloader.* keys that are absent
            if "dataloader.dp_rank_" in str(e):
                self.state_dict.pop("dataloader", None)
                return super().create_local_plan()
            else:
                logger.info("Not entering there, printing: ")
                logger.info(e)
            raise
...
class PTDCheckpointer(BaseCheckpointer):
    ...
    def load(self, step: int = -1) -> bool:
        ...
        #TODO (Bryan): Solve the Skipmissing data loader error
        #planner = SkipMissingDataloaderPlanner()
        #torch.distributed.checkpoint.load(states, checkpoint_id=checkpoint_dir.as_posix(), planner=planner)
        ...
        #TODO (Bryan): Casting Optimizers to its required datatype
        if step != 0 and "optimizer" in states:
            #states["optimizer"].cast_state_to_param_dtype()
            #early initi the optimizers to make sure everything is working
            states["optimizer"].eager_initialize_adam_state()
