...
class ControlTrainer(Trainer):
    ...
    def _prepare_models(self) -> None:
        ...
        # TODO(BRYAN): allow multiple control conditions instead of just one if there's a use case for it
        control_number = 1
        new_in_features = self.model_specification._original_control_layer_in_features \
                    + control_number * self.model_specification._vae_output_channel
        ...
        #TODO (Bryan): Erase the dtype report helper
        self._dtype_report("after_load")
    ...
    def _prepare_trainable_parameters(self) -> None:
        ...
        if (
            ...
            logger.info("Applying layerwise casting modules")
            ...
            logger.info("Layerwise upcasting modules are not applied")
        ...
        if parallel_backend.data_sharding_enabled:
            logger.info("entering parallel backend")
            self.transformer.to(dtype=self.args.transformer_dtype)
        else:
            logger.info("not entering parallel backend")
            if self.args.training_type == TrainingType.CONTROL_LORA:
                logger.info("entering control lora cast params to float32")
    ...
    def _prepare_for_training(self) -> None:
        ...
        self.state.train_state = TrainState()
        #TODO (Bryan): Erase the dtype report helper
        self._dtype_report("before optimizer")
        ...
        #TODO (Bryan): Erase the dtype report helper
        self._dtype_report("after optimizer")
    ...
    def _prepare_dataset(self) -> None:
        ...
        # ─── BRYAN DEBUG: log the first batch ────────────────────────────────────
        logger.info("Trying to debug first batch content")
        # try:
        #     it = iter(self.dataloader)
        #     first_batch = next(it)
        #     # first_batch is typically a dict (or tuple) of tensors/names
        #     logger.info(f"First batch contents:\n{first_batch}")
        # except StopIteration:
        #     logger.warning("Dataloader returned no batches (empty dataset).")
        # except Exception as e:
    ...
    def _prepare_checkpointing(self) -> None:
        ...
        if resume_from_checkpoint is not None:
            ...
            #TODO (Bryan): Erase the dtype report helper
            self._dtype_report("after ckpt load")
    ...
    def _train(self) -> None:
        ...
        # logger.info("Reach testing wiat for everyone")
        # parallel_backend.wait_for_everyone()
        # logger.info("Reach testing wiat for everyone passed")
        ...
        #TODO (Bryan): Brute force setting Compute Posterior to True
        #compute_posterior = False if self.args.enable_precomputation else (not self.args.precomputation_once)
        compute_posterior = True

        
        logger.info("Value of compute_posterior = ")
        logger.info(compute_posterior)
        ...
        while (
            ...
            #TODO (Bryan): Remove Brute force downcast to BFloat16
            ...
            try:
                ...
                print("Someone enter stop iteration!")
                ...
                print("Someone enter other error")
                print(e)
                raise e
            if sampler.is_ready:
            ...
            if train_state.step % self.args.gradient_accumulation_steps == 0:
                ...
                # TODO(Bryan): Remove dtype report before stepping optimizer
                #self._dtype_report("before stepping optimizer")
                self.optimizer.step()
                self.lr_scheduler.step()
    ...
    #TODO (Bryan): Remove the final args
    def _validate(self, step: int, final_validation: bool = False, run_from_eval: bool = False) -> None:
        ...
        while True:
            ...
            ORIGINAL_IMAGE_PATH = validation_data.get("granular_image_file", None)
            ...
            for index, (key, artifact) in enumerate(list(artifacts.items())):
                ...
                filename += f"{step}-{rank}-{index}-{ORIGINAL_IMAGE_PATH}.{ext}"
        ...
        if parallel_backend.is_main_process:
            ...
            #TODO (Bryan): Clean the branch later
            if not run_from_eval:
        ...
        if not final_validation:
            self._move_components_to_device()
            logger.info("Self transformer eval is in train mode")
            self.transformer.train()
    ...
    def _init_pipeline(self, final_validation: bool = False) -> DiffusionPipeline:
        ...
        if not final_validation:
            ...
            pipeline = self.model_specification.load_pipeline(
                ...
                force_every_validation_to_be_bfloat16=self.args.force_every_validation_to_be_bfloat16,
            )
        else:
            self._delete_components()
            ...
            logger.info("Enable slicing: ")
            logger.info(self.args.enable_slicing)
            logger.info("Enable tiling: ")
            logger.info(self.args.enable_tiling)
            logger.info("enable_model_cpu_offload: ")
            logger.info(self.args.enable_model_cpu_offload)
            pipeline = self.model_specification.load_pipeline(
    ...
    def _prepare_data(
        ...
        if not self.args.enable_precomputation:
            ...
            logger.info("Consume latent done")
            self._delete_components(component_names)
            del latent_components, component_names, component_modules
            logger.info("Delete components done")
    ...
    # ── DEBUG: writes a compact dtype report to a text file ────────────────
    # TODO (Bryan): THis is just a dtype report function ,erase if no longer needed
    def _dtype_report(self, tag: str, sample: int = 3):
        """
        Append a dtype summary to output_dir/dtype_report.txt
        (tag = where in the lifecycle we call it)
        """
        if self.optimizer is None:
            return                                # nothing to log yet

        outfile = Path(self.args.output_dir) / "dtype_report.txt"
        outfile.parent.mkdir(parents=True, exist_ok=True)

        total_cnt, total_mem = {}, {}
        mismatches = []          # (shape, param_dtype, state_dtype)

        # helper
        def _acc(key, n_elem, dtype):
            bytes_ = n_elem * torch.tensor([], dtype=dtype).element_size()
            total_cnt[key]  = total_cnt.get(key, 0)  + n_elem
            total_mem[key]  = total_mem.get(key, 0)  + bytes_

        for opt in self.optimizer.optimizers:
            for group in opt.param_groups:
                for p in group["params"]:
                    n = p.numel()
                    _acc(f"param.{p.dtype}", n, p.dtype)

                    st  = opt.state[p]
                    ea  = st.get("exp_avg",    None)
                    ea2 = st.get("exp_avg_sq", None)

                    if ea  is not None: _acc(f"exp_avg.{ea.dtype}",  n, ea.dtype)
                    if ea2 is not None: _acc(f"exp_sq.{ea2.dtype}",  n, ea2.dtype)

                    if ea is not None and ea.dtype != p.dtype and len(mismatches) < sample:
                        mismatches.append((tuple(p.shape), p.dtype, ea.dtype))

        with outfile.open("a") as f:
            f.write(f"\n[{tag}] dtype summary\n")
            for k in sorted(total_cnt):
                cnt = total_cnt[k] / 1e6          # million elems
                mem = total_mem[k] / 1024**2      # MB
                f.write(f"  {k:<18}: {cnt:8.2f} M  {mem:8.1f} MB\n")

            if mismatches:
                f.write(f"  ⚠ mismatches (param vs exp_avg), first {len(mismatches)}:\n")
                for shape, pd, sd in mismatches:
                    f.write(f"    {shape}  param={pd}  state={sd}\n")
