...
class IterableControlDataset(torch.utils.data.IterableDataset, torch.distributed.checkpoint.stateful.Stateful):
    def __init__(
        ...
        if control_type == ControlType.CANNY:
            ...
            logger.info("Entering control type others")
    ...
    def _run_custom_control_processors(self, data:Dict[str, Any]) -> Dict[str, Any]:
        ##TODO (BRYAN): Hard code the height and width for now
        num_frames, num_channels, height, width = data["video"].shape
        height = 480
        width = 720


        ## TODO:(BRYAN) this control right now is only assumed to be Video based
        data["control"], _first_frame_only = FF.resize_to_nearest_bucket_video(
                    data["control"], [[num_frames, height, width]], resize_mode="bicubic"
                )
        data["video"], _first_frame_only = FF.resize_to_nearest_bucket_video(
                    data["video"], [[num_frames, height, width]], resize_mode="bicubic"
                )
        data["image"] = FF.resize_to_nearest_bucket_image(
                    data["image"], [[height, width]], resize_mode="bicubic"
                )
        return data
    def _run_custom_control_guidance_interp_processors(self, data:Dict[str, Any]) -> Dict[str, Any]:
        ##TODO (BRYAN): Hard code the height and width for now
        num_frames, num_channels, height, width = data["video"].shape
        height = 480
        width = 720
        ...
        ## TODO:(BRYAN) this control right now is only assumed to be Video based
        data["guidance_control"], _first_frame_only = FF.resize_to_nearest_bucket_video(
                    data["guidance_control"], [[num_frames, height, width]], resize_mode="bicubic"
                )
        data["sketch_control"], _first_frame_only = FF.resize_to_nearest_bucket_video(
                    data["sketch_control"], [[num_frames, height, width]], resize_mode="bicubic"
                )
        data["video"], _first_frame_only = FF.resize_to_nearest_bucket_video(
                    data["video"], [[num_frames, height, width]], resize_mode="bicubic"
                )
    ...
    def _run_control_processors(self, data: Dict[str, Any]) -> Dict[str, Any]:
        if self.control_type == ControlType.CUSTOM:
            return self._run_custom_control_processors(data)
        elif self.control_type == ControlType.CUSTOM_GUIDANCE_INTERP:
            return self._run_custom_control_guidance_interp_processors(data)
...
class ValidationControlDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        if control_type == ControlType.CANNY:
            ...
            logger.info("Validation Entering control type others")
    ...
    def _run_custom_control_processors(self, data:Dict[str, Any]) -> Dict[str, Any]:
        ##DO NOT RESHAPE ANYTHING, AS IT IS DONE IN THE VALIDATION LAYER
        return data
...
def apply_frame_conditioning_on_latents(
    ...
    if frame_conditioning_type == FrameConditioningType.INDEX:
        logger.info("Index frame conditioning entered")
        ...
        indexing = [slice(None)] * latents.nfdim
