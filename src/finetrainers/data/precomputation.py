...
class PrecomputedDistributedDataPreprocessor(DistributedDataProcessorMixin):
    def __init__(
        ...
        self._num_items = num_items
        print("num items here are: ", self._num_items)
        self._processor_fn = processor_fn
    ...
    def consume(
        ...
        if not self._enable_reuse:
            logger.info("not enable reuse entered")
        ...
        if self._enable_reuse:
            ...
            #TODO (Bryan): DO NOT HARDCODE THE NUM ITEMS
            data_iterator._num_items = self._num_items
...
class PrecomputedDataIterable:
    ...
    def __iter__(self) -> Iterable[Dict[str, Any]]:
        ...
        logger.info("num items are: ")
        logger.info(self._num_items)
        for i in range(self._num_items):
