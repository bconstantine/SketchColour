...
class ImageToVideoControlGuidanceInterpFilelistDataset(
    torch.utils.data.IterableDataset, torch.distributed.checkpoint.stateful.Stateful):
    VALID_CAPTION_FILES = ["prompts.txt", "prompt.txt", "captions.txt", "caption.txt"]
    VALID_VIDEO_LISTS  = ["videos.txt", "video.txt"]
    VALID_VIDEO_FOLDERS = ["video", "videos"]
    VALID_IMAGE_LISTS  = ["images.txt", "image.txt"]
    VALID_IMAGE_FOLDERS = ["image", "images"]
    VALID_SKETCH_CONTROL_LISTS = ["sketches.txt"]
    VALID_SKETCH_CONTROL_FOLDERS = ["sketches"]
    VALID_GUIDANCE_CONTROL_LISTS = ["guidances.txt"]
    VALID_GUIDANCE_CONTROL_FOLDERS = ["guidances"]
    def __init__(self, root: str, infinite: bool = False, only_starting_image:bool = True) -> None:
        super().__init__()
        ...
        self.image_name_list_file = self._pick_one(self.VALID_IMAGE_LISTS, "images")
        self.image_folder_name = self._pick_one(self.VALID_IMAGE_FOLDERS, "image folders")
        self.sketch_control_name_list_file = self._pick_one(self.VALID_SKETCH_CONTROL_LISTS, "sketch control")
        self.sketch_control_folder_name = self._pick_one(self.VALID_SKETCH_CONTROL_FOLDERS, "sketch control folders")
        self.guidance_control_name_list_file = self._pick_one(self.VALID_GUIDANCE_CONTROL_LISTS, "guidance control")
        self.guidance_control_folder_name = self._pick_one(self.VALID_GUIDANCE_CONTROL_FOLDERS, "guidance control folders")
        ...
        self.image_name_list = self._load_image_name_data(self.image_name_list_file, 
                                                          self.image_folder_name, 
                                                          self.only_starting_image)
        self.sketch_control_name_list = self._load_sketch_control_name_data(self.sketch_control_name_list_file, self.sketch_control_folder_name)
        self.guidance_control_name_list = self._load_guidance_control_name_data(self.guidance_control_name_list_file, self.guidance_control_folder_name)

        assert len(self.caption_list) == len(self.video_name_list) == len(self.image_name_list) == len(self.sketch_control_name_list) == len(self.guidance_control_name_list)

        # ---------------------------------------------------------
        # 3. Build HuggingFace `datasets.Dataset` with typed columns
        # ---------------------------------------------------------
        records: List[Dict[str, str]] = []
        for i, (prompt, vid, img, sketch_ctrl, guidance_ctrl) in enumerate(zip(self.caption_list, 
                                              self.video_name_list, 
                                              self.image_name_list, 
                                              self.sketch_control_name_list, 
                                              self.guidance_control_name_list)):
            rec: Dict[str, str] = {"caption": prompt, "video": vid, 
                                   "image":img, "control": sketch_ctrl, "guidance_ctrl":guidance_ctrl}
            records.append(rec)
    ...
    def _load_image_name_data(self, filename, imgfoldername, only_starting_image):
        ...
        if only_starting_image:
            ...
            img_filenames = [(str(self.root / imgfoldername / a),
                            ...
                            for a, b in zip(img_filename[::2], img_filename[1::2])]
        return img_filenames
    def _load_sketch_control_name_data(self, filename, sketchctrlfoldername):
        ctrl_filename = self._read_lines(self.root / filename)
        return [str(self.root / sketchctrlfoldername / p) for p in ctrl_filename]
    def _load_guidance_control_name_data(self, filename, guidancectrlfoldername):
        ctrl_filename = self._read_lines(self.root / filename)
        return [str(self.root / guidancectrlfoldername / p) for p in ctrl_filename]
...
def initialize_dataset(
    ...
    *,
    ...
    assert dataset_type in ["image", "video", "i2v-control", "i2v-control-guidance-interp"]
    ...
    elif dataset_type in ["i2v-control-guidance-interp"]:
        return _custom_initialize_guidance_interp_local_dataset(
            dataset_name_or_root, dataset_type, infinite, _caption_options=_caption_options
        )
...
def _custom_initialize_guidance_interp_local_dataset(
    dataset_name_or_root: str,
    dataset_type: str,
    infinite: bool = False,
    *,
    ...
    dataset = ImageToVideoControlGuidanceInterpFilelistDataset(root.as_posix(), infinite=infinite)
